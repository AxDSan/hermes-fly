...
clear_runtime_overrides() {
  unset WHATSAPP_ENABLED WHATSAPP_MODE WHATSAPP_ALLOWED_USERS WHATSAPP_HOME_CHANNEL WHATSAPP_HOME_CONTACT
...
for line in env_path.read_text(encoding='utf-8').splitlines():
    ...
    key = key.strip()
    if key in wanted | {'WHATSAPP_HOME_CHANNEL', 'WHATSAPP_HOME_CONTACT'}:
...
if self_number:
    ...
    print(f"export WHATSAPP_HOME_CHANNEL={shlex.quote(self_number)}")
